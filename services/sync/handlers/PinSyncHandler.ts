...
export class PinSyncHandler extends BaseSyncHandler<Pin, RePin, typeof pins> {
  ...
  protected async postSync(localUpserts: Pin[], remoteUpserts: RePin[]): Promise<void> {
    ...
    await this.handleImageUploads(remoteUpserts);
    await this.handleImageDownloads(localUpserts);
    await this.handleImageUpdates(localUpserts, remoteUpserts);
    await this.handleImageDeletions(localUpserts, remoteUpserts);
  }
